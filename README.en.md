...
| `agent-master doctor` | Diagnose why sessions fail even though startup looked fine, with a fix for each finding |
...
> Command output is currently localized in Simplified Chinese.

## Troubleshooting

When `start` and `status` both look healthy but sessions still fail, run:

```bash
agent-master doctor
```

It checks every precondition a session depends on — whether the daemon is
serving, whether a stale process is holding the port, whether the `claude`
binary is present, whether credentials resolve, and whether the login-shell
environment was imported — and prints a runnable fix for each failure, along
with the log path and recent errors. It exits `1` when something is broken, so
it works in a script or CI check.

Note that "the daemon is running" and "a session can run" are different claims.
`/health` reports both: `status` for whether the process is serving, and `ready`
for whether a session can be expected to succeed. Clients should drive their
availability indicator from the latter.
...
- `shell_env_keys`: variable **names** resolved by the last successful login-shell
  probe (names only, never values); maintained automatically by the daemon
- `shell_env_optional`: when `true`, keep running sessions even if shell credential
  variables that used to be readable no longer are
...
internal/provider/       Claude Code CLI driver and credential detection
internal/shellenv/       login-shell environment resolution
